...
只在任务具有明显阶段、步骤依赖、较长执行过程、不确定性或需要中途校验时使用 update_plan；简单、明确、短时任务直接执行。不要创建单步计划，也不要为了满足形式把一个简单动作拆成多个步骤。update_plan 只记录本次运行的轻量进度，开始步骤时标记 in_progress，完成后立即写入 completed 并推进下一步。
