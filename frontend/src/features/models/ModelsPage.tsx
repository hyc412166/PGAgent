...
import { AlertCircle, LoaderCircle, Pencil, Plus, RefreshCw, Sparkles, Trash2 } from 'lucide-react'
import { type FormEvent, useState } from 'react'
import { api, describeError } from '../../api'
import { resolveEffectiveThinking, sessionThinkingOptions } from '../../composerSettings'
import { allConnectionModels, availableConnectionModels } from '../../modelSettings'
...
function ModelsPage() {
  ...
  const connectionMutationBusy = saving || Boolean(discovering || togglingConnectionId || updatingThinkingId || updatingModelKey || deletingConnectionId)
  ...
  async function saveConnection(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload: Record<string, unknown> = { name: form.get('name'), base_url: form.get('base_url'), provider: form.get('provider'), api_protocol: form.get('api_protocol'), default_model: manualModel || null, manual_models: manualModel ? [manualModel] : [], thinking_level: form.get('thinking_level') }
  ...
  async function discover(connection: Connection) {
    ...
    try {
      const saved = await api.post<Connection>(`/api/connections/${connection.id}/discover`)
  ...
  // 单击模型标签即可切换启用状态；后端负责同步清理失效的默认、会话和 Agent 选择。
  async function toggleModel(connection: Connection, model: string, enabled: boolean) {
    if (connectionMutationBusy) return
    const modelKey = `${connection.id}:${model}`
    const disabledModels = new Set(connection.disabled_models || [])
    if (enabled) disabledModels.delete(model)
    else disabledModels.add(model)
    setUpdatingModelKey(modelKey)
    setDiscoveryError((value) => ({ ...value, [connection.id]: '' }))
    try {
      const saved = await api.patch<Connection>(`/api/connections/${connection.id}`, { disabled_models: [...disabledModels] })
      connections.setState({ data: connections.data.map((item) => item.id === saved.id ? saved : item), loading: false, error: '' })
    } catch (error) {
      setDiscoveryError((value) => ({ ...value, [connection.id]: describeError(error) }))
    } finally {
  ...
  return (
    <div className="page">
        ...
        <section className="connection-list">
          {connections.data.map((connection) => (
            <article className="connection-card" key={connection.id}>
              ...
              <dl><div><dt>Base URL</dt><dd>{connection.base_url || '—'}</dd></div><div><dt>API 协议</dt><dd>{connection.api_protocol === 'chat_completions' ? 'Chat Completions' : 'Responses'}</dd></div><div><dt>API Key</dt><dd>{connection.api_key_hint || '已安全保存'}</dd></div><div><dt>默认模型</dt><dd>{connection.default_model || '尚未选择'}</dd></div></dl>
              <div className="model-block"><div className="model-block-head"><strong>可用模型</strong><button className="button button-quiet" onClick={() => discover(connection)} disabled={connectionMutationBusy}>{discovering === connection.id ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}发现模型</button></div>{allConnectionModels(connection).length ? <div className="model-tags">{allConnectionModels(connection).map((model) => { const enabled = availableConnectionModels(connection).includes(model); const modelKey = `${connection.id}:${model}`; return <button type="button" className={`model-tag${enabled ? ' is-enabled' : ''}`} aria-pressed={enabled} title={enabled ? `停用 ${model}` : `启用 ${model}`} disabled={connectionMutationBusy} onClick={() => void toggleModel(connection, model, !enabled)} key={modelKey}>{updatingModelKey === modelKey && <LoaderCircle className="spin" size={11} />}{model}</button> })}</div> : <p className="muted">尚未获取模型列表，可点击发现或使用手动模型 ID。</p>}</div>
              <div className="thinking-row"><div><strong>思考强度</strong><small>不同提供商会映射到各自支持的参数</small></div><select aria-label={`${connection.name} 思考强度`} value={resolveEffectiveThinking(connection.thinking_level)} disabled={connectionMutationBusy} onChange={(event) => void updateThinking(connection, event.target.value)}>{sessionThinkingOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>
      ...
      {panelOpen && <SlidePanel title={editingConnection ? '修改模型 API' : '添加模型连接'} description={editingConnection ? '修改连接信息；API Key 留空会继续使用已安全保存的密钥。' : '保存后会检测模型目录和所选 API 协议。'} onClose={closeConnectionPanel}><form key={editingConnection?.id || 'new-connection'} className="panel-form" onSubmit={saveConnection} autoComplete="off"><div className="form-row"><Field label="连接名称"><input name="name" required placeholder="例如：DeepSeek" defaultValue={editingConnection?.name || ''} autoFocus /></Field><Field label="提供商"><select name="provider" defaultValue={editingConnection?.provider || 'openai_compatible'}><option value="openai_compatible">OpenAI 兼容</option><option value="openrouter">OpenRouter</option><option value="deepseek">DeepSeek</option></select></Field><Field label="API 协议"><select name="api_protocol" defaultValue={editingConnection?.api_protocol || 'responses'}><option value="responses">Responses（推荐）</option><option value="chat_completions">Chat Completions</option></select></Field></div><Field label="Base URL"><input name="base_url" type="url" required placeholder="https://api.example.com/v1" defaultValue={editingConnection?.base_url || ''} /></Field><Field label="API Key" hint={editingConnection ? '留空则保持现有密钥；填写新值会覆盖系统凭据' : '只发送给你填写的 Base URL，并写入系统凭据存储'}><input name="api_key" type="password" required={!editingConnection} placeholder={editingConnection ? '留空以保持现有密钥' : 'sk-…'} autoComplete="new-password" /></Field><div className="form-row"><Field label="默认 / 手动模型 ID" hint="当 /models 不可用时也会使用这个模型"><input name="default_model" placeholder="例如：deepseek-chat" defaultValue={editingConnection?.default_model || ''} /></Field><Field label="思考强度"><select name="thinking_level" defaultValue={resolveEffectiveThinking(editingConnection?.thinking_level)}>{sessionThinkingOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field></div>{formError && <p className="form-error" role="alert">{formError}</p>}<div className="form-actions"><button type="button" className="button button-secondary" onClick={closeConnectionPanel}>取消</button><button className="button button-primary" disabled={connectionMutationBusy}>{saving && <LoaderCircle className="spin" size={15} />}{editingConnection ? '保存修改' : '保存并检测'}</button></div></form></SlidePanel>}
    </div>
  )
}
