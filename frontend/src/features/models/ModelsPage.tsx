...
import { EmptyState, ErrorState, Field, LoadingState, PageHeader, SlidePanel, StatusBadge } from '../../components/ui'
...
function ModelsPage() {
  ...
  async function deleteConnection(connection: Connection) {
    if (deletingConnectionId || !window.confirm(`确定删除模型连接“${connection.name}”吗？使用它的 Agent 和会话会改为自动选择其他可用连接，此操作不可撤销。`)) return
  ...
  return (
    <div className="page">
      <PageHeader eyebrow="模型舱" title="模型设置" description="配置 Base URL 与 API Key，PGAgent 会发现模型并统一思考强度。" action={<button className="button button-primary" onClick={() => openConnectionPanel()}><Plus size={16} />添加连接</button>} />
        ...
        <section className="connection-list">
          {connections.data.map((connection) => (
            <article className="connection-card" key={connection.id}>
              <header><div className="provider-icon"><Sparkles size={20} /></div><div><h2>{connection.name}</h2><p>{connection.provider || 'OpenAI Compatible'}</p></div><div className="connection-card-actions"><StatusBadge status={connection.status || 'unknown'} /><button type="button" className="icon-button" aria-label={`编辑 ${connection.name}`} title="修改模型 API" onClick={() => openConnectionPanel(connection)}><Pencil size={15} /></button><button type="button" className="icon-button danger-icon" aria-label={`删除 ${connection.name}`} title="删除模型 API" disabled={deletingConnectionId === connection.id} onClick={() => void deleteConnection(connection)}>{deletingConnectionId === connection.id ? <LoaderCircle className="spin" size={15} /> : <Trash2 size={15} />}</button></div></header>
