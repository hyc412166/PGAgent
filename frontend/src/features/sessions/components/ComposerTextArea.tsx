...
import { alignComposerValueState, type ComposerValueState } from './composerTextAreaState'
...
export const ComposerTextArea = forwardRef<ComposerTextAreaHandle, { disabled: boolean; resetKey: string; placeholder: string; onHasValueChange: (hasValue: boolean) => void }>(function ComposerTextArea({ disabled, resetKey, placeholder, onHasValueChange }, ref) {
  const [valueState, setValueState] = useState<ComposerValueState>({ resetKey, value: '' })
  const textareaRef = useRef<HTMLTextAreaElement>(null)
  const hasValueRef = useRef(false)
  const alignedValueState = alignComposerValueState(valueState, resetKey)
  if (alignedValueState !== valueState) setValueState(alignedValueState)
  const value = alignedValueState.value
  const updateValue = useCallback((nextValue: string) => {
    setValueState({ resetKey, value: nextValue })
